...
func _open_mobile_draft_sheet(cid: String) -> void:
    ...
    # P49.2 — DRAFT MOBILE : CTA "AJOUTER À L'ÉQUIPE" rendu indépendamment
    # du thème Button. Le helper _button() force flat=true : on ne l'utilise
    # donc PAS ici pour le cadre visuel du CTA.
    ...
    info_scroll.size = Vector2(804,408)
    ...
    _label(info_panel,reason,Rect2(28,434,780,34),15,reason_color,HORIZONTAL_ALIGNMENT_CENTER,true)

    var add_disabled: bool = _scheduled_team() != "ally" or not _draft_allowed("ally",data)

    # Cadre VISUEL indépendant : impossible à masquer par Button.flat.
    var cta_frame := Panel.new()
    cta_frame.position = Vector2(58,480)
    cta_frame.size = Vector2(720,132)
    cta_frame.mouse_filter = Control.MOUSE_FILTER_IGNORE
    cta_frame.z_index = 5

    var cta_style := StyleBoxFlat.new()
    cta_style.bg_color = Color(0.028,0.205,0.118,1.0) if not add_disabled else Color(0.055,0.072,0.062,0.98)
    cta_style.border_color = Color(0.38,1.0,0.66,1.0) if not add_disabled else Color(0.32,0.43,0.36,0.86)
    cta_style.set_border_width_all(5)
    cta_style.set_corner_radius_all(18)
    cta_style.shadow_color = Color(0,0,0,0.68)
    cta_style.shadow_size = 12
    cta_style.shadow_offset = Vector2(0,6)
    cta_frame.add_theme_stylebox_override("panel",cta_style)
    info_panel.add_child(cta_frame)

    # Zone tactile transparente AU-DESSUS du cadre.
    var add := Button.new()
    add.position = cta_frame.position
    add.size = cta_frame.size
    add.text = "AJOUTER À L'ÉQUIPE"
    add.flat = true
    add.focus_mode = Control.FOCUS_ALL
    add.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
    add.z_index = 6
    add.add_theme_font_size_override("font_size",28)
    add.add_theme_color_override("font_color",Color("f7fff9"))
    add.add_theme_color_override("font_hover_color",Color("ffffff"))
    add.add_theme_color_override("font_pressed_color",Color("ffffff"))
    add.add_theme_color_override("font_focus_color",Color("ffffff"))
    add.add_theme_color_override("font_disabled_color",Color("aab8af"))
    add.disabled = add_disabled
    add.tooltip_text = "AJOUTER À L'ÉQUIPE"
    info_panel.add_child(add)

    add.button_down.connect(func() -> void:
        if not add.disabled:
            cta_frame.modulate = Color(0.80,0.90,0.84,1.0)
            _play_pick(-13.0)
    )
    add.button_up.connect(func() -> void:
        cta_frame.modulate = Color.WHITE
    )
