...
func _on_economy_state_updated(_state: Dictionary) -> void:
    if current_screen == "shop":
        call_deferred("_show_shop",false)
...
func _shop_ownership_label(data: Dictionary) -> String:
    ...
    return "POSSÉDÉE DÉFINITIVEMENT • DE BASE" if stars <= 3.0 else "NON POSSÉDÉE"
...
func _show_shop(sync_server: bool = true) -> void:
    _destroy_home_overlay()
    current_screen = "shop"
    _clear_screen()
    if sync_server:
...
func _show_shop_mobile_like_draft() -> void:
    ...
    var weekly_source: String = "AUTH SERVER" if _shop_weekly_free_ids().size() > 0 else "INDISPONIBLE"
    _capsule_in(
        screen_root,
        Rect2(54,430,214,30),
        ("%d GRATUITES" % weekly_count),
    ...
    footer_notice.text = "Boutique : 3★ possédées de base • toutes les cartes visibles restent consultables • économie Auth Server."
...
func _build_shop_mobile_canvas(scroll: ScrollContainer) -> void:
    ...
    for data: Dictionary in shop_mobile_cards:
        ...
        # Le filtre de possession ne doit JAMAIS bloquer l'ouverture d'une fiche.
        # Seuls recherche/rôle peuvent rendre une carte réellement non-interactive.
        interactive_by_id[cid] = base_filters_match
