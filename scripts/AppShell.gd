...
func _show_profile() -> void:
    ...
    _screen_heading(
        "PROFIL SHINOBI",
        "Identité YUGITO • statistiques classées • profil commun PC / Mobile."
    )
...
func _show_play() -> void:
    ...
    _screen_heading(
        "JOUER",
        "Solo, Multijoueur Classic ou Entraînement."
    )

    _mode_card(
        Rect2(42,124,458,500),
        "SOLO VS IA",
        "COMPLET",
    ...
    _mode_card(
        Rect2(553,124,458,500),
        "MULTIJOUEUR",
        "CLASSIC",
        "Rejoindre un salon, créer une partie ou lancer le matchmaking classé avec ton compte YUGITO.",
        Color("58aff0"),
        Callable(self,"_show_multiplayer_hub"),
        "02"
    )
    _mode_card(
        Rect2(1064,124,458,500),
        "ENTRAÎNEMENT",
        "À VENIR",
        "Bac à sable pour choisir librement les Ninjas et reproduire rapidement une situation de combat.",
        Color("ba85ed"),
        Callable(self,"_training_placeholder"),
        "03"
    )

    footer_notice.text = "Solo complet • Multijoueur Classic prêt côté interface."
...
func _show_multiplayer_hub() -> void:
    ...
    var connected: bool = IdentityManager.is_account_connected()
    var status_color := Color("55d58b") if connected else Color("e0b34f")

    _screen_heading(
        "MULTIJOUEUR CLASSIC",
        "Rejoindre • Créer • Matchmaking classé • Parties privées"
    )

    # Header réseau / identité.
    _panel_in(
        screen_root,
        Rect2(30,100,1500,92),
        Color(0.010,0.026,0.045,0.96),
        Color(0.34,0.62,0.78,0.34),
        18,
        7
    )
    _label_in(
    ...
    _label_in(
        screen_root,
        "ELO %d" % int(IdentityManager.profile().get("elo",100)),
        Rect2(58,154,200,24),
        12,
        Color("e4c96c"),
        HORIZONTAL_ALIGNMENT_LEFT,
    ...
    _capsule_in(
        screen_root,
        Rect2(1290,126,200,30),
        "COMPTE PRÊT" if connected else "CONNEXION REQUISE",
        status_color
    )

    _multiplayer_mode_panel(
        Rect2(30,214,470,430),
        "REJOINDRE",
        "SALONS PUBLICS",
        "Affiche les salons YUGITO Classic disponibles.\n\nChoisis une partie publique puis rejoins directement la salle d'attente avant le Draft.",
        Color("58aff0"),
        "LISTE DES SALONS",
        Callable(self,"_multiplayer_join_screen")
    )

    _multiplayer_mode_panel(
        Rect2(545,214,470,430),
        "CRÉER",
        "PARTIE PERSONNALISÉE",
        "Crée un salon YUGITO.\n\nLa future connexion réseau permettra de choisir public / privé puis d'attendre l'adversaire avant de lancer le duel.",
        Color("55d58b"),
        "CRÉER UN SALON",
        Callable(self,"_multiplayer_create_screen")
    )

    _multiplayer_mode_panel(
        Rect2(1060,214,470,430),
        "CLASSÉ",
        "MATCHMAKING ELO",
        "Recherche automatiquement un adversaire classé.\n\nVictoire et défaite alimenteront le même profil ELO YUGITO sur PC et Mobile.",
        Color("d8b050"),
        "LANCER LA RECHERCHE",
        Callable(self,"_multiplayer_ranked_screen")
    )

    var private_btn := _button_in(
        screen_root,
        Rect2(474,660,610,54),
        "INVITATIONS PRIVÉES • ESPACE SHINOBI",
        Color("ba85ed"),
        true
    )
    private_btn.add_theme_font_size_override("font_size",14)
    private_btn.pressed.connect(_multiplayer_open_private_invites)

    footer_notice.text = "Multijoueur : interface Classic prête • transport réseau encore à connecter."
...
func _multiplayer_mode_panel(rect: Rect2, title: String, subtitle: String, description: String, accent: Color, action_text: String, callback: Callable) -> void:
    ...
    _label_in(
        screen_root,
        title,
        Rect2(rect.position+Vector2(28,28),Vector2(rect.size.x-56,44)),
        28,
        Color("ffffff"),
        HORIZONTAL_ALIGNMENT_LEFT,
        true
    ...
    _label_in(
        screen_root,
        subtitle,
        Rect2(rect.position+Vector2(28,78),Vector2(rect.size.x-56,30)),
        13,
        accent,
        HORIZONTAL_ALIGNMENT_LEFT,
        true
    ...
    _panel_in(
        screen_root,
        Rect2(rect.position+Vector2(24,126),Vector2(rect.size.x-48,196)),
    ...
    _rich_text_in(
        screen_root,
        description,
        Rect2(rect.position+Vector2(46,148),Vector2(rect.size.x-92,150)),
        14,
        Color("b8cad7")
    )
    var btn := _button_in(
        screen_root,
        Rect2(rect.position+Vector2(28,344),Vector2(rect.size.x-56,58)),
        action_text,
        accent,
        true
    )
...
func _multiplayer_join_screen() -> void:
    _destroy_home_overlay()
    current_screen = "multiplayer_join"
    _clear_screen()
    _screen_heading("REJOINDRE UNE PARTIE","Liste des salons publics YUGITO Classic.")

    _panel_in(screen_root,Rect2(120,140,1320,500),Color(0.010,0.026,0.045,0.96),Color(0.34,0.62,0.78,0.34),20,8)
    _label_in(screen_root,"SALONS DISPONIBLES",Rect2(170,180,500,42),26,Color("ffffff"),HORIZONTAL_ALIGNMENT_LEFT,true)
    _label_in(
        screen_root,
        "Aucun faux salon n'est généré localement.\n\nCette liste sera alimentée par le relais multijoueur YUGITO dès que le transport Godot sera connecté.",
        Rect2(270,310,1020,140),
        19,
        Color("9db1c0"),
        HORIZONTAL_ALIGNMENT_CENTER,
        false
    )
    var back := _button_in(screen_root,Rect2(560,530,440,66),"RETOUR MULTIJOUEUR",Color("58aff0"),true)
    back.add_theme_font_size_override("font_size",17)
    back.pressed.connect(_show_multiplayer_hub)
    footer_notice.text = "REJOINDRE : attente du transport réseau réel."

func _multiplayer_create_screen() -> void:
    _destroy_home_overlay()
    current_screen = "multiplayer_create"
    _clear_screen()
    _screen_heading("CRÉER UNE PARTIE","Préparation d'un salon YUGITO Classic.")

    _panel_in(screen_root,Rect2(240,150,1080,470),Color(0.010,0.026,0.045,0.96),Color(0.32,0.72,0.48,0.34),20,8)
    _label_in(screen_root,"NOUVEAU SALON",Rect2(300,194,500,42),28,Color("ffffff"),HORIZONTAL_ALIGNMENT_LEFT,true)
    _label_in(screen_root,"TYPE",Rect2(300,270,180,30),16,Color("9db1c0"),HORIZONTAL_ALIGNMENT_LEFT,true)

    var public_btn := _button_in(screen_root,Rect2(300,314,430,70),"PUBLIC",Color("55d58b"),true)
    public_btn.disabled = true
    var private_btn := _button_in(screen_root,Rect2(830,314,430,70),"PRIVÉ",Color("ba85ed"),true)
    private_btn.disabled = true

    _label_in(
        screen_root,
        "Le serveur réel attribuera l'identifiant du salon et gérera l'attente de l'adversaire.\nAucune salle fictive n'est créée sur le téléphone.",
        Rect2(340,430,880,90),
        17,
        Color("9db1c0"),
        HORIZONTAL_ALIGNMENT_CENTER,
        false
    )
    var back := _button_in(screen_root,Rect2(560,540,440,62),"RETOUR MULTIJOUEUR",Color("55d58b"),true)
    back.pressed.connect(_show_multiplayer_hub)
    footer_notice.text = "CRÉER : interface prête • création réseau à connecter."

func _multiplayer_ranked_screen() -> void:
    _destroy_home_overlay()
    current_screen = "multiplayer_ranked"
    _clear_screen()

    var data := IdentityManager.profile()
    _screen_heading("MATCHMAKING CLASSÉ","Recherche ELO YUGITO.")

    _panel_in(screen_root,Rect2(280,144,1000,486),Color(0.010,0.026,0.045,0.96),Color(0.78,0.62,0.24,0.38),22,8)
    _label_in(screen_root,"TON ELO",Rect2(560,190,440,40),22,Color("d8b050"),HORIZONTAL_ALIGNMENT_CENTER,true)
    _label_in(screen_root,str(int(data.get("elo",100))),Rect2(560,238,440,100),68,Color("ffffff"),HORIZONTAL_ALIGNMENT_CENTER,true)
    _label_in(
        screen_root,
        "Le matchmaking cherchera un adversaire de niveau proche.\nL'ELO est un classement, jamais une monnaie à dépenser.",
        Rect2(390,370,780,100),
        18,
        Color("b8cad7"),
        HORIZONTAL_ALIGNMENT_CENTER,
        false
    )
    var search := _button_in(screen_root,Rect2(480,494,600,70),"RECHERCHER UN ADVERSAIRE",Color("d8b050"),true)
    search.disabled = true
    search.add_theme_font_size_override("font_size",18)
    var back := _button_in(screen_root,Rect2(610,578,340,42),"RETOUR",Color("71879a"),false)
    back.pressed.connect(_show_multiplayer_hub)
    footer_notice.text = "CLASSÉ : écran prêt • matchmaking réseau à connecter."
...
func _show_collection_mobile_like_draft() -> void:
    ...
    _screen_heading(
        "COLLECTION",
        "%d/70 Ninjas • glisse comme dans le Draft • touche une carte pour ouvrir sa fiche." % collection_mobile_cards.size()
    )
    ...
    footer_notice.text = "Collection mobile : catalogue Canvas identique au Draft • une texture atlas • fiche complète au toucher."
...
func _build_collection_mobile_canvas(scroll: ScrollContainer) -> void:
    ...
    for data: Dictionary in collection_mobile_cards:
        ...
        available_by_id[cid] = matches
        interactive_by_id[cid] = matches
        reason_by_id[cid] = "" if matches else "HORS FILTRE"
...
func _show_deck_builder_mobile_like_draft() -> void:
    ...
    var scroll := ScrollContainer.new()
    scroll.position = Vector2(434,238)
    scroll.size = Vector2(766,468)
    scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
    scroll.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_AUTO
    scroll.scroll_deadzone = 10
...
func _build_deck_mobile_canvas(scroll: ScrollContainer) -> void:
    ...
    for data: Dictionary in deck_mobile_cards:
        ...
        if not matches:
            ...
            # Même une carte interdite reste consultable pour expliquer la règle.
            interactive_by_id[cid] = true
...
func _show_shop_mobile_like_draft() -> void:
    ...
    _capsule_in(screen_root,Rect2(1260,620,240,36),"ÉCONOMIE À CONNECTER",Color("e0b84e"))

    footer_notice.text = "Boutique mobile : même moteur Atlas que le Draft • aucune fausse transaction locale."
...
func _build_shop_mobile_canvas(scroll: ScrollContainer) -> void:
    ...
    for data: Dictionary in shop_mobile_cards:
        var cid: String = str(data.get("id",""))
        var matches: bool = _card_matches_filters(data,shop_search_query,shop_role_filters)
        available_by_id[cid] = matches
        interactive_by_id[cid] = matches
        reason_by_id[cid] = "" if matches else "HORS FILTRE"
...
func _refresh_shop_mobile_count() -> void:
    ...
    var matches: int = 0
    for data: Dictionary in shop_mobile_cards:
        if _card_matches_filters(data,shop_search_query,shop_role_filters):
            matches += 1
...
""" % [
    ...
    var stars: float = float(data.get("stars",0.0))
    var price: int = _shop_price_yt(stars)
    var price_label: String = _shop_price_label(stars)
    var free_card: bool = price <= 0
    var cta_accent: Color = Color("55d58b") if free_card else Color("d9b64e")
    ...
    var cta := _button_in(
        cta_panel,
        Rect2(12,10,812,62),
        ("RÉCUPÉRER • GRATUIT" if free_card else "ACHETER • %s" % price_label),
        cta_accent,
        true
    )
    cta.add_theme_font_size_override("font_size",18)
    ...
    _label_in(
        cta_panel,
        ("3★ : carte gratuite • récupération à connecter au compte YUGITO." if free_card
        else "PRIX : %s • achat YT à connecter au solde et à la possession du compte." % price_label),
        Rect2(20,74,796,24),
        10,
