...
# P53 — Collection mobile : seulement 3 cartes lourdes en mémoire par page.
var collection_mobile_page: int = 0
const COLLECTION_MOBILE_PAGE_SIZE: int = 3
var collection_detail_overlay: Control = null
...
func _clear_screen() -> void:
    if screen_root == null:
        return
    collection_detail_overlay = null
...
func _show_collection_mobile() -> void:
    _screen_heading(
        "COLLECTION",
        "%d/70 Ninjas • cartes grand format • touche une carte pour ouvrir sa fiche complète." % sorted_cards.size()
    )

    _panel_in(
        screen_root,
        Rect2(24, 94, 1508, 622),
        Color(0.010,0.025,0.042,0.88),
        Color(0.36,0.58,0.75,0.30),
        18,
        8
    )

    var search := LineEdit.new()
    search.position = Vector2(42, 108)
    search.size = Vector2(520, 54)
    search.placeholder_text = "Rechercher un Ninja, élément, rôle, technique…"
    search.text = collection_search_query
    ...
    collection_count_label = _label_in(
        screen_root, "", Rect2(590, 113, 430, 42),
        14, Color("dbe8f2"), HORIZONTAL_ALIGNMENT_LEFT, true
    )

    var clear_btn := _button_in(
        screen_root, Rect2(1342, 108, 166, 54),
        "EFFACER", Color("70879d"), true
    )
    clear_btn.add_theme_font_size_override("font_size", 13)
    clear_btn.pressed.connect(_clear_collection_filters)

    # Tous les rôles restent disponibles, mais dans UNE barre horizontale :
    # plus de 20 petits boutons compressés sur plusieurs lignes.
    var role_scroll := ScrollContainer.new()
    role_scroll.position = Vector2(42, 174)
    role_scroll.size = Vector2(1466, 58)
    ...
    role_scroll.mouse_filter = Control.MOUSE_FILTER_PASS
    ...
    for role: String in roles:
        ...
        rb.custom_minimum_size = Vector2(154,48)
        rb.add_theme_font_size_override("font_size", 11)
        rb.pressed.connect(_toggle_collection_role.bind(role))

    # Zone cartes : UNE seule page de 3 cartes grand format.
    collection_grid = GridContainer.new()
    collection_grid.position = Vector2(178, 244)
    collection_grid.size = Vector2(1200, 414)
    collection_grid.columns = 3
    collection_grid.add_theme_constant_override("h_separation", 28)
    collection_grid.add_theme_constant_override("v_separation", 0)
    screen_root.add_child(collection_grid)

    _refresh_collection_grid()

    var prev := _button_in(screen_root, Rect2(42, 660, 188, 46), "◀ PRÉCÉDENT", Color("6c91ad"), true)
    prev.name = "CollectionPrev"
    prev.add_theme_font_size_override("font_size", 12)
    prev.pressed.connect(_collection_mobile_change_page.bind(-1))

    var next := _button_in(screen_root, Rect2(1320, 660, 188, 46), "SUIVANT ▶", Color("6c91ad"), true)
    next.name = "CollectionNext"
    next.add_theme_font_size_override("font_size", 12)
    next.pressed.connect(_collection_mobile_change_page.bind(1))

    footer_notice.text = "COLLECTION MOBILE • 3 cartes chargées par page pour réduire fortement la RAM."

func _collection_mobile_change_page(delta: int) -> void:
    if not MobilePlatform.is_android():
        return
    var filtered := _filtered_collection_cards()
    var page_count: int = maxi(1, int(ceil(float(filtered.size()) / float(COLLECTION_MOBILE_PAGE_SIZE))))
    collection_mobile_page = clampi(collection_mobile_page + delta, 0, page_count - 1)
    _refresh_collection_grid()

func _open_collection_mobile_detail(card_id: String) -> void:
    if not MobilePlatform.is_android():
        return
    ...
    _close_collection_mobile_detail()

    collection_detail_overlay = Control.new()
    collection_detail_overlay.position = Vector2.ZERO
    collection_detail_overlay.size = Vector2(1556, 748)
    collection_detail_overlay.z_index = 2000
    collection_detail_overlay.mouse_filter = Control.MOUSE_FILTER_STOP
    screen_root.add_child(collection_detail_overlay)

    var dim := ColorRect.new()
    dim.position = Vector2.ZERO
    dim.size = collection_detail_overlay.size
    dim.color = Color(0.004,0.010,0.018,0.93)
    dim.mouse_filter = Control.MOUSE_FILTER_STOP
    collection_detail_overlay.add_child(dim)

    var accent: Color = _element_color(str(data.get("element","")))
    var win := _panel_in(
        collection_detail_overlay,
        Rect2(42, 26, 1472, 696),
        Color(0.012,0.027,0.044,0.985),
    ...
    _label_in(
        win, str(data.get("name","NINJA")).to_upper(),
        Rect2(36,22,820,48), 30, Color("ffffff"),
        HORIZONTAL_ALIGNMENT_LEFT, true
    )
    _label_in(
        win,
        "%s★  •  %s  •  PV %d  •  TAI %d  •  NIN %d  •  GEN %d" % [
            _stars_text(float(data.get("stars",0.0))),
            str(data.get("element","")).to_upper(),
            int(data.get("hp",0)),
            int(data.get("taijutsu",0)),
            int(data.get("ninjutsu",0)),
            int(data.get("genjutsu",0))
        ],
        Rect2(38,72,1040,34), 15, accent.lightened(0.18),
        HORIZONTAL_ALIGNMENT_LEFT, true
    )

    var close_btn := _button_in(
        win, Rect2(1246, 24, 188, 60),
        "FERMER", Color("86bfe4"), true
    )
    close_btn.add_theme_font_size_override("font_size", 15)
    close_btn.pressed.connect(_close_collection_mobile_detail)

    # Illustration intégrale : pas de crop agressif.
    var art_panel := _panel_in(
        win, Rect2(36, 122, 430, 526),
        Color(0.006,0.016,0.027,0.98),
    ...
    art.size = Vector2(410,506)
    var art_path: String = "res://assets/cards/%s_field.png" % card_id
    art.texture = AssetCache.texture(art_path) if ResourceLoader.exists(art_path) else null
    ...
    var info_panel := _panel_in(
        win, Rect2(492, 122, 942, 526),
        Color(0.010,0.025,0.042,0.93),
        Color(0.38,0.56,0.72,0.34),
        16
    )

    var scroll := ScrollContainer.new()
    scroll.position = Vector2(24,20)
    scroll.size = Vector2(894,486)
    scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
    info_panel.add_child(scroll)

    var rich := RichTextLabel.new()
    rich.custom_minimum_size = Vector2(856,760)
    rich.bbcode_enabled = true
    rich.fit_content = true
    rich.scroll_active = false
    rich.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
    rich.add_theme_font_size_override("normal_font_size", 15)
    rich.add_theme_color_override("default_color",Color("e2edf5"))
    rich.text = """[color=#8bc8ee][font_size=18][b]RÔLES[/b][/font_size][/color]
%s

[color=#63d7a5][font_size=19][b]PASSIF — %s[/b][/font_size][/color]
%s

[color=#e8c85d][font_size=19][b]TECHNIQUE SPÉCIALE — %s[/b][/font_size][/color]
%s

[color=#69baf0][font_size=19][b]SYNERGIES[/b][/font_size][/color]
%s
""" % [
        ...
        SynergyDB.description(card_id,cards_by_id)
    ]
    scroll.add_child(rich)

func _close_collection_mobile_detail() -> void:
    if collection_detail_overlay != null and is_instance_valid(collection_detail_overlay):
        collection_detail_overlay.queue_free()
    collection_detail_overlay = null
...
func _refresh_collection_grid() -> void:
    ...
    if MobilePlatform.is_android():
        var page_count: int = maxi(1, int(ceil(float(filtered.size()) / float(COLLECTION_MOBILE_PAGE_SIZE))))
        collection_mobile_page = clampi(collection_mobile_page, 0, page_count - 1)

        var start_i: int = collection_mobile_page * COLLECTION_MOBILE_PAGE_SIZE
        var end_i: int = mini(filtered.size(), start_i + COLLECTION_MOBILE_PAGE_SIZE)
        for i: int in range(start_i, end_i):
            collection_grid.add_child(_collection_tile(filtered[i]))

        if collection_count_label:
            collection_count_label.text = "%d résultat%s  •  PAGE %d/%d  •  %d filtre%s" % [
                filtered.size(),
                "" if filtered.size() == 1 else "s",
                collection_mobile_page + 1,
                page_count,
                collection_role_filters.size(),
                "" if collection_role_filters.size() == 1 else "s"
            ]

        var prev := screen_root.get_node_or_null("CollectionPrev") as Button
        var next := screen_root.get_node_or_null("CollectionNext") as Button
        if prev != null:
            prev.disabled = collection_mobile_page <= 0
        if next != null:
            next.disabled = collection_mobile_page >= page_count - 1
        return
...
func _collection_tile(data: Dictionary) -> Button:
    ...
    var card_size: Vector2 = Vector2(372, 408) if MobilePlatform.is_android() else Vector2(286, 420)
    card.setup(
        data,
        card_size,
        cid == collection_selected_id,
        false,
        "TOUCHER POUR OUVRIR • " + role_text if MobilePlatform.is_android() else (role_text if not role_text.is_empty() else "OUVRIR LA FICHE")
    )

    if MobilePlatform.is_android():
        card.pressed.connect(_open_collection_mobile_detail.bind(cid))
    else:
