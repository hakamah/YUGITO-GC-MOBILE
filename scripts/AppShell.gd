...
func _ready() -> void:
    ...
    if not SocialManager.data_changed.is_connected(_on_social_data_changed):
        SocialManager.data_changed.connect(_on_social_data_changed)
...
func _show_social() -> void:
    ...
    var unread := SocialManager.total_unread()
...
func _draw_social_chat_p59() -> void:
    ...
    var friend := SocialManager.get_friend(social_selected_peer_id)
    if friend.is_empty():
...
func _draw_social_requests_p59() -> void:
    ...
    var rows := SocialManager.incoming_list()
    if rows.is_empty():
...
func _draw_social_invites_p59() -> void:
    ...
    for i in range(SocialManager.pending_invites.size()):
        ...
        var left := SocialManager.invite_seconds_left(i)
...
func _social_remove_friend(peer_id: String) -> void:
    var friend := SocialManager.get_friend(peer_id)
...
func _social_toggle_mute(peer_id: String) -> void:
    var state := SocialManager.toggle_mute(peer_id)
...
func _social_toggle_block(peer_id: String) -> void:
    var state := SocialManager.toggle_block(peer_id)
