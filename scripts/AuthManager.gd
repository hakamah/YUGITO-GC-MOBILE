...
signal economy_state_received(data: Dictionary)
signal weekly_collection_received(data: Dictionary)
signal economy_purchase_received(card_id: String, data: Dictionary)
signal economy_request_failed(kind: String, message: String)
...
func refresh_economy_state() -> void:
    if session_token.is_empty():
        economy_request_failed.emit("state","Compte YUGITO non connecté.")
        return
    _request_json(
        "economy_state",
        HTTPClient.METHOD_GET,
...
func refresh_weekly_collection() -> void:
    # Route publique : elle doit fonctionner même avant/restauration de session.
    _request_json(
        "weekly_collection",
        HTTPClient.METHOD_GET,
        "/api/collection/weekly"
    )

func purchase_economy_card(card_id: String) -> void:
    if session_token.is_empty():
        economy_request_failed.emit("purchase","Compte YUGITO non connecté.")
        return
    var cid: String = card_id.strip_edges()
    if cid.is_empty():
        economy_request_failed.emit("purchase","Carte invalide.")
        return
    _request_json(
        "economy_purchase:" + cid,
        HTTPClient.METHOD_POST,
...
func _on_request_completed(
    ...
    match kind:
        ...
        "economy_state":
            economy_state_received.emit(data)
        "weekly_collection":
            weekly_collection_received.emit(data)
        _:
            if kind.begins_with("economy_purchase:"):
                economy_purchase_received.emit(kind.trim_prefix("economy_purchase:"),data)
            elif kind == "logout":
                pass
...
func _handle_http_error(kind: String, response_code: int, data: Dictionary) -> void:
    ...
    if kind in ["economy_state","weekly_collection"] or kind.begins_with("economy_purchase:"):
        var economy_kind: String = "purchase" if kind.begins_with("economy_purchase:") else ("weekly" if kind == "weekly_collection" else "state")
        economy_request_failed.emit(economy_kind,message)
        return
