...
var _plan_badge_panel: Panel
var _plan_badge_label: Label
...
func consume_special() -> void:
    if card_id == "ino":
        ...
        # Classic : C'était prévu ! revient tous les deux tours de Tobi.
        # On utilise un vrai cooldown réutilisable, pas le verrou special_used.
        special_used = false
        special_cooldown = 2
        status_tags.erase("cooldown_skip_tick")
...
func _build_card(data: Dictionary) -> Control:
    ...
    # P50.4 — A1 / A2 / AF font désormais partie de la carte elle-même.
    # Même ligne que les étoiles : étoiles à gauche, plan d'action à droite.
    _plan_badge_panel = _pill(root, Rect2(145, 14, 92, 23), Color(0.025,0.055,0.042,0.97), Color("52d6a0"), 6)
    _plan_badge_panel.z_index = 22
    _plan_badge_panel.visible = false
    _plan_badge_panel.mouse_filter = Control.MOUSE_FILTER_IGNORE
    _plan_badge_label = _label(_plan_badge_panel, "", Rect2(3,0,86,22), 10, Color("baffdf"), HORIZONTAL_ALIGNMENT_CENTER, true)
    _plan_badge_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
...
func status_lines() -> Array[Dictionary]:
    ...
    if special_cooldown > 0:
        var total: int = 2 if card_id == "tobi" else (3 if card_id in ["ino","karin"] else (4 if card_id in ["shikamaru","konohamaru","gengetsu"] else special_cooldown))
...
func set_action_plan_badges(labels: Array[String]) -> void:
    if _plan_badge_panel == null or _plan_badge_label == null:
        return
    if labels.is_empty():
        _plan_badge_panel.visible = false
        _plan_badge_label.text = ""
        return

    _plan_badge_label.text = " • ".join(labels)

    var accent: Color = Color("52d6a0")
    if labels.has("AF"):
        accent = Color("e2b746")
    elif labels.has("A2"):
        accent = Color("f2a350")

    var style: StyleBoxFlat = _plan_badge_panel.get_theme_stylebox("panel") as StyleBoxFlat
    if style != null:
        style.bg_color = Color(accent.r * 0.15, accent.g * 0.15, accent.b * 0.15, 0.98)
        style.border_color = Color(accent.r, accent.g, accent.b, 1.0)
        style.set_border_width_all(2)
        style.shadow_color = Color(accent.r, accent.g, accent.b, 0.20)
        style.shadow_size = 4

    _plan_badge_label.add_theme_color_override("font_color", accent.lightened(0.24))
    _plan_badge_panel.visible = true

func clear_action_plan_badges() -> void:
    if _plan_badge_panel != null:
        _plan_badge_panel.visible = false
    if _plan_badge_label != null:
        _plan_badge_label.text = ""
