...
signal duel_result(victory: bool, reason: String, turns: int, duration_seconds: float)
...
func _apply_mobile_battle_layout(plan_glass: Panel) -> void:
    ...
    # P50.5 — vrai CTA mobile validé visuellement.
    validate_button = _mobile_validate_button(Rect2(1318, 428, 258, 92))
    validate_button.pressed.connect(_on_validate_plan_pressed)
...
func _build_action_markers() -> void:
    # P50.5 — plus aucun A1/A2/AF flottant sur le terrain.
    # Le rendu est entièrement confié à CardActor.gd.
    action_marker_labels.clear()
...
func _refresh_action_markers() -> void:
    ...
    for actor: YugitoCardActor in card_actors:
        if actor == null or not is_instance_valid(actor) or actor.team_name != "ally":
            continue
        if actor.defeated:
            actor.clear_action_plan_badges()
            continue

        var slot: int = actor.seed_index_value()
        var labels: Array[String] = []
        if tags.has(slot):
            for value: Variant in tags[slot]:
                labels.append(str(value))
        actor.set_action_plan_badges(labels)
...
func _refresh_inspection_actions(actor: YugitoCardActor) -> void:
    ...
    for key: Variant in action_buttons.keys():
        ...
        if action_key in ["taijutsu", "ninjutsu", "genjutsu"]:
            ...
            # P51.1 — retour visuel immédiat de la règle :
            # si ce Ninja possède déjà une attaque normale dans l'autre slot,
            # ses 3 arts normaux sont grisés dans sa fiche.
            enabled = enabled and not _normal_action_rule_blocked(actor, action_key, planning_slot)
        elif action_key == "special":
            # La spéciale reste totalement indépendante du quota normal.
...
func set_duel_reward_text(value: String) -> void:
    duel_reward_text = value
...
func _show_duel_end_overlay(victory: bool, reason: String) -> void:
    ...
    if not duel_reward_text.is_empty():
        _label_in(win,duel_reward_text,Rect2(100,202,700,32),16,Color("e7c75b"),HORIZONTAL_ALIGNMENT_CENTER,true)
    ...
    _label_in(win,summary,Rect2(100,244,700,64),11,Color("c6d6e3"),HORIZONTAL_ALIGNMENT_CENTER,false)
    var journal_preview := RichTextLabel.new()
    journal_preview.position = Vector2(80,322)
...
func _on_card_selection_requested(actor: YugitoCardActor) -> void:
    ...
    if current_action != "" and selected_actor != null and selected_actor.team_name == "ally" and actor.team_name == "enemy" and not actor.defeated:
        ...
        if current_is_special and current_special_id in ["neji", "hinata"]:
            ...
            _refresh_action_buttons()
            return
        # P51.3 — AF/Hiraishin automatique :
        # la cible est maintenant connue, donc on peut choisir l'art qui inflige
        # réellement le plus de dégâts à CETTE carte.
        if current_action == "free_auto":
            var best_style: String = _best_normal_style_against_target(selected_actor, actor)
            if best_style.is_empty():
                _battle_log_set("Hiraishin : aucune attaque normale disponible pour Minato.")
                free_action_mode = false
                current_action = ""
                _refresh_action_buttons()
                return
            current_action = "free_%s" % best_style
            action_status_label.text = "HIRAISHIN • %s automatique" % _action_display_name(best_style)
            _battle_log_set("Hiraishin choisit automatiquement %s contre %s." % [_action_display_name(best_style), actor.display_name])
...
func _is_normal_combat_action(action_id: String) -> bool:
    var normalized: String = action_id.trim_prefix("free_")
    return normalized in ["taijutsu", "ninjutsu", "genjutsu"]

func _descriptor_is_same_ninja(descriptor: Dictionary, actor: YugitoCardActor) -> bool:
    if descriptor.is_empty() or actor == null or not is_instance_valid(actor):
        return false

    # Une vraie instance terrain possède un UID stable.
    var descriptor_uid: int = int(descriptor.get("source_uid", 0))
    if descriptor_uid > 0 and actor.battle_uid > 0:
        return descriptor_uid == actor.battle_uid

    # Les acteurs virtuels utilisés après un Switch A1 n'ont pas encore d'UID :
    # on les distingue alors par carte + slot.
    return (
        str(descriptor.get("source_id", "")) == actor.card_id
        and int(descriptor.get("source_slot", -999)) == actor.seed_index_value()
    )

func _other_slot_has_normal_for_ninja(actor: YugitoCardActor, slot_being_planned: int) -> bool:
    if actor == null or not is_instance_valid(actor):
        return false

    var descriptors: Array[Dictionary] = []
    if slot_being_planned != 1:
        descriptors.append(action1_plan)
    if slot_being_planned != 2:
        descriptors.append(action2_plan)

    for descriptor: Dictionary in descriptors:
        if descriptor.is_empty():
            continue
        if str(descriptor.get("kind", "attack")) != "attack":
            continue
        if not _is_normal_combat_action(str(descriptor.get("action_id", ""))):
            continue
        if _descriptor_is_same_ninja(descriptor, actor):
            return true
    return false

func _normal_action_rule_blocked(actor: YugitoCardActor, action_id: String, slot_being_planned: int) -> bool:
    # AF/Hiraishin de Minato est explicitement EXEMPTÉ :
    # free_taijutsu/free_ninjutsu/free_genjutsu ne consomment pas le quota A1/A2.
    if action_id.begins_with("free_"):
        return false
    if not _is_normal_combat_action(action_id):
        return false
    return _other_slot_has_normal_for_ninja(actor, slot_being_planned)

func _normal_action_rule_message(actor: YugitoCardActor) -> String:
    var ninja_name: String = "Ce Ninja" if actor == null else actor.display_name
    return "%s a déjà une attaque normale prévue ce tour. Il peut encore utiliser sa TECHNIQUE SPÉCIALE, mais pas un second TAI/NIN/GEN." % ninja_name
...
func _on_action_button_pressed(action_id: String) -> void:
    ...
    # P51 — une même carte ne peut pas préparer deux attaques normales
    # (TAI/NIN/GEN) dans A1 + A2 pendant le même tour de planification.
    # Normal + spéciale reste autorisé. AF Minato est hors quota.
    if _normal_action_rule_blocked(selected_actor, action_id, planning_slot):
        _battle_log_set(_normal_action_rule_message(selected_actor))
        action_status_label.text = "RÈGLE : 1 SEULE ATTAQUE NORMALE PAR NINJA / TOUR"
        current_action = ""
        _refresh_action_buttons()
        return
...
func _store_planned_action(source: YugitoCardActor, target: YugitoCardActor, action_id: String, extra_metadata: Dictionary = {}) -> void:
    ...
    # P51 — verrou central, y compris pour les attaques directes et les chemins
    # de planification qui ne passent pas directement par le bouton d'action.
    if not is_free and _normal_action_rule_blocked(source, stored_action, planning_slot):
        _battle_log_set(_normal_action_rule_message(source))
        action_status_label.text = "RÈGLE : 1 SEULE ATTAQUE NORMALE PAR NINJA / TOUR"
        current_action = ""
        _refresh_action_buttons()
        return
...
func _best_normal_style_against_target(source: YugitoCardActor, target: YugitoCardActor) -> String:
    if source == null or target == null or not is_instance_valid(source) or not is_instance_valid(target):
        return ""

    var best_style: String = ""
    var best_damage: int = -1

    # Ordre stable en cas d'égalité parfaite.
    for style: String in ["taijutsu", "ninjutsu", "genjutsu"]:
        if not _actor_can_use_style(source, style):
            continue

        # On utilise exactement le calcul de dégâts du moteur contre CETTE cible.
        var calc: Dictionary = _calculate_classic_damage(source, target, style, 0, false, false)
        var damage: int = _preview_true_attack_damage(source, target, int(calc.get("damage", 0)))
...
func _on_hiraishin_pressed() -> void:
    ...
    # P51.3 — AF devient immédiatement un mode de ciblage.
    # Le joueur ne choisit PAS TAI/NIN/GEN : le moteur choisira automatiquement
    # la meilleure attaque normale contre la cible touchée.
    free_action_mode = true
    current_action = "free_auto"
    action_status_label.text = "HIRAISHIN GRATUIT — choisis une cible"
    _battle_log_set("Hiraishin : touche une cible ennemie. Le meilleur art TAI/NIN/GEN sera choisi automatiquement.")
    _refresh_selection_panel()
    _refresh_action_buttons()
...
func _ai_choose_action(primary: bool, previous: Dictionary) -> Dictionary:
    ...
    var previous_normal_uid: int = 0
    var excluded_uid: int = 0
    if not previous.is_empty():
        if str(previous.get("action_id", "")) == "special":
            previous_special_uid = int(previous.get("source_uid", 0))
        if _is_normal_combat_action(str(previous.get("action_id", ""))):
            previous_normal_uid = int(previous.get("source_uid", 0))
    ...
    for actor: YugitoCardActor in _living_cards("enemy"):
        ...
        # P51 — l'IA respecte exactement la même règle que le joueur :
        # si ce Ninja a fait l'attaque normale A1, il ne peut pas refaire
        # TAI/NIN/GEN en A2. Sa spéciale reste parfaitement autorisée.
        if actor.battle_uid != previous_normal_uid:
...
func _check_victory_state() -> void:
    ...
    _refresh_action_buttons()
    duel_result.emit(victory,result_text,turn_counter,elapsed)
...
func _refresh_action_buttons() -> void:
    ...
    for key in action_buttons.keys():
        ...
        elif key in ["taijutsu", "ninjutsu", "genjutsu"] and enabled:
            enabled = _actor_can_use_style(selected_actor, str(key))
            enabled = enabled and not _normal_action_rule_blocked(selected_actor, str(key), planning_slot)
        ...
        disabled_style.bg_color = Color(0.040,0.045,0.052,0.94)
        disabled_style.border_color = Color(0.28,0.31,0.35,0.72)
        disabled_style.set_border_width_all(1)
        disabled_style.set_corner_radius_all(9)
        btn.add_theme_stylebox_override("disabled", disabled_style)
        btn.add_theme_color_override("font_disabled_color", Color("7d8790"))
...
func _mobile_validate_button(rect: Rect2) -> Button:
    var btn := Button.new()
    btn.position = rect.position
    btn.size = rect.size
    btn.text = "VALIDER LE TOUR"
    btn.flat = false
    btn.focus_mode = Control.FOCUS_ALL
    btn.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
    btn.z_index = 180
    btn.add_theme_font_size_override("font_size",20)
    btn.add_theme_color_override("font_color",Color("f7fff9"))
    btn.add_theme_color_override("font_hover_color",Color("ffffff"))
    btn.add_theme_color_override("font_pressed_color",Color("ffffff"))
    btn.add_theme_color_override("font_focus_color",Color("ffffff"))
    btn.add_theme_color_override("font_disabled_color",Color("aab8af"))

    var normal := StyleBoxFlat.new()
    normal.bg_color = Color(0.020,0.190,0.108,0.99)
    normal.border_color = Color(0.34,1.0,0.64,1.0)
    normal.set_border_width_all(4)
    normal.set_corner_radius_all(16)
    normal.shadow_color = Color(0,0,0,0.66)
    normal.shadow_size = 11
    normal.shadow_offset = Vector2(0,6)

    var hover := normal.duplicate() as StyleBoxFlat
    hover.bg_color = Color(0.030,0.255,0.145,1.0)
    hover.border_color = Color(0.50,1.0,0.74,1.0)

    var pressed := normal.duplicate() as StyleBoxFlat
    pressed.bg_color = Color(0.014,0.125,0.075,1.0)
    pressed.shadow_size = 4
    pressed.shadow_offset = Vector2(0,2)

    var disabled := normal.duplicate() as StyleBoxFlat
    disabled.bg_color = Color(0.045,0.060,0.052,0.96)
    disabled.border_color = Color(0.30,0.42,0.34,0.78)
    disabled.shadow_size = 2
...
func _style_minato_af_button(btn: Button) -> void:
    if btn == null:
        return
    btn.flat = false
    btn.add_theme_font_size_override("font_size",19)
    btn.add_theme_color_override("font_color",Color("fff8cf"))
    btn.add_theme_color_override("font_hover_color",Color("ffffff"))
    btn.add_theme_color_override("font_pressed_color",Color("ffffff"))

    var normal := StyleBoxFlat.new()
    normal.bg_color = Color(0.205,0.145,0.020,0.99)
    normal.border_color = Color(1.0,0.82,0.25,1.0)
    normal.set_border_width_all(4)
    normal.set_corner_radius_all(14)
    normal.shadow_color = Color(0,0,0,0.56)
    normal.shadow_size = 8
    normal.shadow_offset = Vector2(0,4)

    var hover := normal.duplicate() as StyleBoxFlat
    hover.bg_color = Color(0.285,0.205,0.030,1.0)
    hover.border_color = Color(1.0,0.90,0.42,1.0)

    var pressed := normal.duplicate() as StyleBoxFlat
    pressed.bg_color = Color(0.145,0.100,0.012,1.0)
    pressed.shadow_size = 3

    var disabled := normal.duplicate() as StyleBoxFlat
    disabled.bg_color = Color(0.075,0.068,0.045,0.96)
    disabled.border_color = Color(0.58,0.48,0.22,0.64)
    disabled.shadow_size = 2
