...
func _apply_mobile_battle_layout(plan_glass: Panel) -> void:
    ...
    # P50.2 : CTA principal mobile remonté sur l'axe droit, proche du centre
    # vertical. On réaffecte validate_button afin que _refresh_plan_ui() garde
    # la main sur son état disabled.
...
func _build_action_markers() -> void:
    # P50.3 : badges A1 / A2 / AF intégrés à la carte, sur la même ligne
    # que l'encadré des étoiles. Le badge occupe la partie droite de cette ligne.
    for slot: int in range(3, 6):
        var marker: Label = _label("", Rect2(0,0,112,24), 11, Color("fff8d2"), HORIZONTAL_ALIGNMENT_CENTER, true)
        marker.z_index = 25000
        marker.visible = false
        marker.clip_text = true
        var marker_style := StyleBoxFlat.new()
        marker_style.bg_color = Color(0.025,0.035,0.048,0.97)
        marker_style.border_color = Color(0.95,0.78,0.28,0.96)
        marker_style.set_border_width_all(2)
        marker_style.set_corner_radius_all(7)
        marker_style.shadow_color = Color(0,0,0,0.48)
        marker_style.shadow_size = 4
        marker_style.shadow_offset = Vector2(0,2)
...
func _refresh_action_markers() -> void:
    ...
    for slot_v: Variant in action_marker_labels.keys():
        ...
        if actor == null or not tags.has(slot):
            lab.visible = false
            continue
        var tag_list: Array = tags[slot] as Array
        lab.text = " • ".join(tag_list)

        # La carte fait 252x396 mais est affichée à ~0,738x sur mobile.
        # La ligne des étoiles est à y=14 dans la carte source, soit environ
        # -136 px par rapport à l'anchor réel. On place le badge sur cette
        # même ligne, à droite des étoiles, directement DANS le cadre.
        lab.position = actor.anchor_position + Vector2(-18, -138)

        var marker_style: StyleBoxFlat = lab.get_theme_stylebox("normal") as StyleBoxFlat
        if marker_style != null:
            var accent := Color("52d6a0")
            if tag_list.has("AF"):
                accent = Color("e2b746")
            elif tag_list.has("A2"):
                accent = Color("f2a350")
            marker_style.border_color = Color(accent.r,accent.g,accent.b,1.0)
            marker_style.bg_color = Color(accent.r * 0.16, accent.g * 0.16, accent.b * 0.16, 0.96)
            lab.add_theme_color_override("font_color",accent.lightened(0.22))
        lab.visible = true
