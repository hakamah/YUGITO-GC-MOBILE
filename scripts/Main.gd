...
func _build_action_markers() -> void:
    # P50.2 : badges locaux collés aux cartes alliées. Plus de texte flottant
    # entre les deux lignes du terrain.
    for slot: int in range(3, 6):
        var marker: Label = _label("", Rect2(0,0,92,34), 14, Color("fff8d2"), HORIZONTAL_ALIGNMENT_CENTER, true)
        marker.z_index = 25000
        marker.visible = false
        marker.clip_text = true
        var marker_style := StyleBoxFlat.new()
        marker_style.bg_color = Color(0.025,0.035,0.048,0.96)
        ...
        marker_style.shadow_color = Color(0,0,0,0.54)
        marker_style.shadow_size = 5
        marker_style.shadow_offset = Vector2(0,3)
...
func _refresh_action_markers() -> void:
    ...
    for slot_v: Variant in action_marker_labels.keys():
        ...
        # La carte fait 252x396 et son anchor est son centre.
        # Le badge est désormais fixé dans son coin supérieur droit, juste
        # à l'intérieur du cadre : il suit donc toujours LA carte concernée.
        lab.position = actor.anchor_position + Vector2(24, -188)
